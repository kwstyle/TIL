...
import AccountGuide from "./scenes/circle/setting/AccountGuide";
...
export default class App extends Component {
    ...
    componentDidMount() {
        ...
        this.notificationUnsubscribe = FCM.on("notification", (notif) => {
            ...
            // TODO 이건도대체..어찌해야할까나..(백그라운드에서도 받고 눌러서 앱실행해도 받으니 확인필요)
            setTimeout(() => {
                console.log("App Status", this.state.currentAppState);
                if (this.state.currentAppState == "active") { // foreground

                } else { // background -> foreground 라도 background 로 인식
                    //EventBus.post("showPushEvent", notif);
                }
            }, 1000);
        });
    ...
    <Scene
        ...
        renderRightButton={()=>{
            return(
                <TouchableOpacity
            onPress={() => {
                EventBus.post("terminationBankAccount");
            }}>
        <Icon
            name="delete"
            size={24}
